...
- OpenAI-compatible LLM Gateway interface (`LLMGateway` / `OmniRouteGateway`)
- Configuration via environment variables: `OMNIROUTE_BASE_URL`, `OMNIROUTE_API_KEY`, `DEFAULT_MODEL`
- Health check available at `GET /api/v1/system/health` returning latency, model, and connectivity status
