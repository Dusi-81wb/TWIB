    ...
    <div className="overflow-x-auto rounded-xl border border-border/80 bg-card">
      <table className="w-full text-left text-xs">
        ...
        <tbody className="divide-y divide-border/60">
          {members.map((mem) => {
            const displayName = mem.name || mem.user_id;
            const displayEmail = mem.email || `${mem.user_id.slice(0, 8)}@twib.ai`;
            return (
              <tr key={mem.id || mem.user_id} className="hover:bg-accent/20 transition-colors">
