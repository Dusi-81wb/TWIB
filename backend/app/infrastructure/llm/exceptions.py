...
Defines provider-independent exception types for LLM integration and gateway calls.
Outer layers can catch these exceptions without depending on provider SDKs.
"""
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            provider: Name of the LLM provider (e.g. 'omniroute', 'openai').
...
class AuthenticationError(LLMProviderError):
    """Raised when authentication against an LLM provider or gateway fails."""
...
class ProviderUnavailableError(LLMProviderError):
    """Raised when an LLM provider or gateway endpoint is unreachable or down."""

    error_code: str = SERVICE_UNAVAILABLE
    status_code: int = status.HTTP_503_SERVICE_UNAVAILABLE


class GatewayError(LLMProviderError):
    """Base exception for gateway-specific failures."""


class GatewayTimeoutError(ProviderUnavailableError):
...
class GatewayUnavailableError(ProviderUnavailableError):
    """Raised when a gateway server is unavailable or returns 502/503/504."""
